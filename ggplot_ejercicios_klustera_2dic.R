...
dir1 <- "/home/rstudio/databases_ggplot" # Base de datos
# dir1 <- "~/nombre_repo/databases" # Base de datos (Solo Docker)
dir2 <- "/home/rstudio/graph" # Gráficas
...
tempo <- data %>%
  filter(year == 2011) %>%
  group_by(sexo) %>%
  summarise(promedio = weighted.mean(tdes, pob))
...
gr <- ggplot(tempo, aes(x=sexo, y=promedio)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label=round(promedio, 2)), vjust=1) +
  labs(title="Tasa de desaparecidos 2011", subtitle = "Por sexo", x="", y="Promedio ponderante")

ggsave(paste(dir2, "graphejercicio_2.png", sep="/"), plot=gr, width=12, height=12)
...
gr <- ggplot(tempo, aes(x=sexo, y=porcentaje, fill=rango_edad)) +
  ...
  geom_text(aes(label=paste0(porcentaje,"%")), position=position_stack(vjust=0.5), angle=1, size=3) +
  ...
  coord_flip() +
  theme_bw()
ggsave(paste(dir2, "graphejercicio_3.png", sep="/"), plot=gr, width=12, height=12)
...
gr <- ggplot(tempo, aes(x=sexo, y=tdes)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle=100,size = 9)) # Elige un ángulo que permita visualizar correctamente el texto

ggsave(paste(dir2, "graphejercicio_6.png", sep="/"), plot=gr, width=20, height=12)

# Ejercicios Klustera:
rm(list=ls())
fklustera <- read_csv("/home/rstudio/datascience-jalisco-b2/e.csv")
fklustera
str(fklustera)
...
tempo <- group_by(fklustera, branch_office) %>%
  summarise(total_conexiones = length(X1))

gr <- ggplot(tempo, aes(x=branch_office, y=total_conexiones)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label=total_conexiones), vjust=1) +
  labs(title="Total de conexiones", subtitle = "Por oficina", x="Oficina", y="Total de conexiones")
...
tempo <- filter(fklustera, visitor == TRUE) %>%
  group_by(day_of_week_tz) %>%
  summarise(total_visitantes = length(X1)) %>%
  arrange(desc(total_visitantes))

gr <- ggplot(tempo, aes(x=day_of_week_tz, y=total_visitantes)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label=total_visitantes), vjust=1) +
  labs(title="Total de visitantes", subtitle = "Por dia de la semana", x="Dia", y="Total de visitantes")
...
tempo <- filter(fklustera, visitor == TRUE) %>%
  group_by(month_tz) %>%
  summarise(personas_x_mes = length(X1))
tempo$month_tz = as.character(tempo$month_tz)
gr <- ggplot(tempo, aes(x=month_tz, y=personas_x_mes)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label=personas_x_mes), vjust=1) +
  labs(title="Total de personas", subtitle = "Por mes", x="Mes", y="Total de personas")
...
tempo <- filter(fklustera, visitor == TRUE) %>%
  group_by(hour_tz) %>%
  summarise(conexiones_x_hora = length(X1))
tempo$labels <- paste(tempo$hour_tz," hrs, ", tempo$conexiones_x_hora)
treemap(tempo, index=c("labels"), vSize="conexiones_x_hora", vColor="index", type="index", title="Horas de más visitantes", palette="Greens", title.legend="", border.col="grey", border.lwd=0.5)
